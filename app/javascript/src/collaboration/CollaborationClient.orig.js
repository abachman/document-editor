...
class CollaborationClient {
  ...
  connectOperationsChannel(documentId, initialVersion) {
    ...
    // immediately request document history after connecting
  ...
  // start doing the transform thing
  ...
  // error occurred on operation sending, request any updates that may have been
  // missed, transform pending, and attempt to resend
  _operationError = (data) => {
    this.inflightOperation = null
    this.operationsChannel.requestOperationsSince(this.version)
    clearTimeout(this.operationTimeout)
    // give server a chance to send pending operations before trying to submit again
    this.operationTimeout = setTimeout(this._submitNextOperation, 10)
  }
